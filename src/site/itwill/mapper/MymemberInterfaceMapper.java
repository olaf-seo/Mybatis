...
//	인터페이스를 이용한 매퍼 파일 - Mybatis Framework 에서만 작성 가능
...
//	@Update : UPDATE 명령을 등록하기 위한 어노테이션
	@Update(value = "update mymember set name=#{name}, phone=#{phone}, email=#{email} where id=#{id}")
	int updateMember(MyMemberDTO member);
	
//	@Delete : DELETE 명령을 등록하기 위한 어노테이션
	@Delete(value = "delete from mymember where id=#{id}")
	int deleteMember(String id);
	
//	@Select : SELECT 명령을 등록하기 위한 어노테이션
	@Select(value = "select * from mymember where id=#{id}")
	MyMemberDTO selectMember(String id);
	
	@Select(value = "select * from mymember order by id")
