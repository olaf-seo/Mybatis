...
//			Resources.getResourceAsStream(String resource) : 리소스(XML) 파일을 읽기 위한 입력스트림을 생성하여 반환하는 메소드
...
//		SqlSessionFactoryBuilder().build(InputStream inputStream) : 환경설정 파일을 참조하여 SqlSessionFactory 인스턴스를 생성하여 반환하는 메소드
		return new SqlSessionFactoryBuilder().build(inputStream);
	}
	
//	STUDENT 테이블에 저장된 모든 학생정보를 검색하여 반환하는 메소드
//	=> SqlSession 인스턴스를 이용하여 Mapper 파일에 등록된 SQL 명령을 전달 실행하고 결과 반환
...
//		SqlSession : Mapper 파일에 등록된 SQL 명령을 전달 실행하고 결과를 자동으로 매핑하여 반환하는 인스턴스
		...
		try {
//			SqlSession.selectList(String elementId) : Mapper 파일에서 SELECT 명령을 얻어와 전달 실행하고 검색결과를 매핑하여 List 인스턴스로 반환하는 메소드
